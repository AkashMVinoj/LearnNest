...
import {
  ...
  ArrowLeft, Edit, Trash2, FileText, Inbox, CheckCircle, Mail, AlertCircle
} from 'lucide-react';
...
/* ============================================
   MENTOR TASKS — with course student-count warning
   ============================================ */
function MentorTasks({ courses }) {
  ...
  const [submitting, setSubmitting] = useState(false);
  const [formError, setFormError] = useState(null);   // ✅ NEW
  const [form, setForm] = useState({
    title: '',
    description: '',
    course: '',
  ...
  // ✅ compute selected course and its enrolled count
  const selectedCourse = courses.find(
    (c) => String(c.id) === String(form.course)
  );
  const selectedCourseEmpty =
    selectedCourse && (selectedCourse.student_count ?? 0) === 0;
          ...
          <Card className="w-full max-w-md p-6">
            ...
            {formError && (
              <div className="mb-4 flex items-start gap-2 p-3 bg-red-50 border border-red-200 rounded-lg">
                ...
                <p className="text-xs text-red-700 break-words">{formError}</p>
              </div>
            )}
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div>
                <label className="block text-sm font-medium text-slate-700 mb-1.5">
                  Course * <span className="text-red-500">(required)</span>
                </label>
                <select
                  required
                  value={form.course}
                ...
                >
                  ...
                  {courses.map((c) => (
                    <option key={c.id} value={c.id}>
                      {c.title}
                      {c.student_count != null ? ` (${c.student_count} student${c.student_count === 1 ? '' : 's'})` : ''}
                    </option>
                  ))}
                </select>

                {/* ✅ live warning when the picked course has no students */}
                {selectedCourseEmpty && (
                  <div className="mt-2 flex items-start gap-2 p-2.5 bg-amber-50 border border-amber-200 rounded-lg">
                    <AlertCircle className="w-4 h-4 text-amber-600 mt-0.5 flex-shrink-0" />
                    <p className="text-xs text-amber-800">
                      This course has <strong>no enrolled students</strong> yet. The task won't appear on anyone's page until someone enrolls.
                    </p>
                  </div>
                )}
              </div>
...
/* ============================================
   MENTOR SUBMISSIONS — grade via /auth/tasks/<id>/
   ============================================ */
